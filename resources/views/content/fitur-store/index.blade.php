        ...
        <div class="card">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-hover">
                    <thead><tr><th>#</th><th data-sortable="false">Fitur</th><th>Status</th><th>Dibuat</th><th>Aksi</th></tr></thead>
                    <tbody class="table-border-bottom-0">
                        @forelse($fiturStores as $item)
                        <tr>
                            <td>{{ $loop->iteration + ($fiturStores->currentPage() - 1) * $fiturStores->perPage() }}</td>
                            <td><small class="text-muted">{{ Str::limit(implode(', ', $item->feature_items) ?: '-', 140) }}</small></td>
