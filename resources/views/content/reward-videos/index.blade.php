...
<div class="row">
    <div class="col-12">
        ...
        <div class="card">
            ...
            <div class="card-body border-bottom pb-4 pt-3">
                <form method="GET" action="{{ route('reward-videos.index') }}" class="row g-3 align-items-end">
                    ...
                    <div class="col-auto d-flex gap-2">
                        ...
                        @if(request()->filled('status'))<a href="{{ route('reward-videos.index') }}" class="btn btn-sm btn-outline-secondary">Reset</a>@endif
                    </div>
                </form>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover">
                    <thead><tr><th>#</th><th>Preview</th><th>Judul</th><th>Status</th><th>Dipakai</th><th>Aksi</th></tr></thead>
                    <tbody class="table-border-bottom-0">
                        @forelse($rewardVideos as $video)
                        <tr>
                            <td>{{ $loop->iteration + ($rewardVideos->currentPage() - 1) * $rewardVideos->perPage() }}</td>
                            <td><video src="{{ $video->video_src }}" muted style="width:96px;height:54px;object-fit:cover;" class="rounded bg-label-secondary"></video></td>
                            <td>
                                <span class="fw-medium">{{ $video->title }}</span>
                                @if($video->video_path)<small class="text-muted d-block">Upload</small>@endif
                            </td>
                            <td>@if($video->status)<span class="badge bg-label-success">Aktif</span>@else<span class="badge bg-label-secondary">Nonaktif</span>@endif</td>
            ...
            @if($rewardVideos->hasPages())<div class="card-footer d-flex justify-content-end">{{ $rewardVideos->links() }}</div>@endif
        </div>
    </div>
</div>
@endsection
